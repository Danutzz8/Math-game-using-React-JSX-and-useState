function App() {
    ...
    React.useEffect(() => {
      if (score == 10 || mistakes == 3) {
        setTimeout(() => resetButton.current.focus(), 300)
      }
    }, [score, mistakes])
    ...
    function handleSubmit(e) {
      ...
      if (correctAnswer == parseInt(userAnswer, 10)) {
        setScore((prev) => prev + 1)           // incrementing 
        setCurrentProblem(generateProblem())
        setUserAnswer("")  // after answering the imput will become cleare
      } else {
        setMistakes((prev) => prev + 1) // incrementing
        setShowError(true) // this is what will turn the style to apear red on a wrong answer and to do that in react we need to use setTimeout
        setTimeout(() => setShowError(false), 401) // will be put back on default after .4s
      }
    }
  
    function resetGame() {    // this will reastart the game once more
      setScore(0)
      setMistakes(0)
      setUserAnswer("")
      setCurrentProblem(generateProblem())
    ...
    return (
      <div>
        <div
          className={"main-ui" + (mistakes == 3 || score == 10 ? " blurred" : "")} // blurred background image
        >
          ...
          <form onSubmit={handleSubmit} action="" className="our-form">
            <input
              ref={answerField} // this will refocus the answear area once clicked on the outside
              value={userAnswer}
